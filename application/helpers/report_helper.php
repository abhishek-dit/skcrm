...
/* Opportunity Lost REPORT START 
   Created By : Prasad
   Description : This Report explains about reasons for loosing opportunity in the current financial year. To which competitor we have lost that opportunity.
   ...
   Logic : In this report , First Pie Chart contains the reasons for loosing opportunity.
   Second pie chart tells about to which  competitor we have lost that opportunity.
   this method gives information regarding opportunity lost reasons.
...
function getOpportunityLostChart1DataReason($reporty_by=1,$searchFilters)
{
	...
	/*Fetching all opportuntiy reasons with opportunity status 7,8
	*/
...
/* Opportunity Lost REPORT START 
   ...
   Logic : In this report , First Pie Chart contains the reasons for loosing opportunity.
   Second pie chart tells about to which  competitor we have lost that opportunity.
   this method gives information regarding to which competitor we have lost that opportunity.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees
   Prerequisites : Financial year should be there to view this report.
*/
function getOpportunityLostChart1DataCompetitor($reporty_by=1,$searchFilters)
{
	...
	$searchFilters['reportee_users']=$user_id;
	/*
	 Fetching all competitor data with opportunity status 7,8
	*/
...
/* Opportunity Lost REPORT
   ...
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , We will show region wise opportunity lost segment data for selected reason or opportunity
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees,lost for
   Prerequisites : Financial year should be there to view this report.
*/
function getOpportunityLostChart2Data($lost_for,$report_by,$searchFilters)
{
    ...
    {	
		case 1:
		    /* Fetching region wise categories data with opportunity status 7,8 */
		    // foreach ($product_groups as $pg) {
			//$c1Data=array();
				   
				$region=$CI->Report_model->get_user_location_regions();
				$xAxisCategory=array();
				$segment_arr=array();
...
/* Opportunity Lost REPORT
   ...
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , we will show customer list based on the selected data 
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees,lost for,region,segment
   Prerequisites : Financial year should be there to view this report.
*/
function getOpportunityLostChart3Data($lost_for,$region,$segment,$report_by,$searchFilters)
{
	...
	$series = array();
	switch($report_by)
	{   /* Fetching customer data based on selected category and region with opportunity 7,8 */
		case 1:
...
function getFreshBusinessChart1Data($searchFilters)
{
	    ...
	    $CI=& get_instance();
	    $xAxis=array();
	    $regions=$this->Report_model->get_user_location_regions();
...
/* Open Order REPORT START 
   Created By : Prasad
   Description : This Report explains about open orders in the current financial year.
   open order means contract note is generated but still in approval process.
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report ,We are showing the results in category wise.Fresh Open Order column indicates contract note 
   is generated but it is waiting  for RBH Approval.Fresh not cleared indicates RBH has given approval and but waiting for SO Entry open.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees
   Prerequisites : Financial year should be there to view this report.
*/
 function getOpenOrderChart1Data($searchFilters)
{
 //fetching different groups from product category
    ...
    $product_category=$CI->Common_model->get_data('product_category',array('status',1,'company_id'=>$CI->session->userdata('company')));
    $c1Data=array();
	$c2Data=array();
	$xAxisCategory=array();
	...
	foreach ($product_category as $pc) {
			/* fetching cnote list with status 1,3 (waiting for cnote clearance and so entry)
			   fresh cleared  --- rbh approval is done and waiting for so entry
			   fresh not cleared -- Waiting for cnote approval 
			*/
		    ...
		    if($searchFilters['vtime']=='m'||$searchFilters['vtime']=='q' || $searchFilters['vtime']=='y')
		    {    
				//fetching previous open order details
...
/* Open Order REPORT 2nd level
   Created By : Prasad
   Description : This Report explains about open orders in the current financial year.
   open order means contract note is generated but still in approval process.
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , We are showing segment wise open orders for the selected category and series.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees,category_name,series_name
   Prerequisites : Financial year should be there to view this report.
*/
function getOpenOrderChart2Data($searchFilters,$status,$category)
{
	...
	if($searchFilters['vtime']=='w'||$searchFilters['vtime']=='y')
	{
		$colors=array('#FFD54F','#3F51B5','#7ABA7A','#FF9800','#F44336', '#4CAF50', '#9C27B0', '#795548', '#FFEB3B', '#42A5F5', '#CDDC39', '#A1887F', '#99b3ff', '#CC1559', '#6D929B', '#e87d7d', '#bea7a7', '#d9ff66','#717D8C', '#66ff8c');
		$i=0;
		//fetching open orders list segment wise
...
/* Open Order REPORT 3rd level
   Created By : Prasad
   Description : This Report explains about open orders in the current financial year.
   open order means contract note is generated but still in approval process.
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , We are showing customer ,product wise open orders for the selected category and series,segment.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees,category_name,series_name,segment
   Prerequisites : Financial year should be there to view this report.
*/
function getOpenOrderChart3Data($searchFilters,$status,$category,$segment)
{
	...
	$colors=array('#FFD54F','#3F51B5','#7ABA7A','#FF9800','#F44336', '#4CAF50', '#9C27B0', '#795548', '#FFEB3B', '#42A5F5', '#CDDC39', '#A1887F', '#99b3ff', '#CC1559', '#6D929B', '#e87d7d', '#bea7a7', '#d9ff66','#717D8C', '#66ff8c');
	$i=0;
	//fetching customer wise open order list 
   ...
   //fetching product wise open orders list
...
/* Target vs Sales REPORT START 
   ...
   he can reach the target or not in this respective timeline.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees
   Prerequisites : Financial year should be there to view this report.
*/
...
{   
    ...
    if($searchFilters['zone']==1)//graph
    {
	    ...
	    //fetching current sales(with status 1)
	    ...
	    //fetching open orders(with status 2 ,3)
	    ...
	    //fetching open opportunites with status 1,2,3,4,5
	...
	{   
		...
		{
			$location_wise = $CI->Report_model->get_user_location_regions();
...
{   
	...
	{   
		...
		case "y":
			...
			// $q = 'date('.$parameter.') BETWEEN "'.$start.'" AND "'.$end.'" ';
			$f_year = explode('-',$searchFilters['duration']);
 		    // $start = $fy_year['start_date'];
			 // $end = $fy_year['end_date'];
			$start = $f_year[0].'-04-01';
			$end = isset($f_year[1])?$f_year[1].'-03-31':$fy_year['end_date'];
 			$q = 'date('.$parameter.') BETWEEN "'.$start.'" AND "'.$end.'" ';
			break;
		case "w":
		    $start=$fy_dates['start_date'];
...
{   
	...
	$CI->db->where('f.company_id',$CI->session->userdata('company'));
	$res=$CI->db->get();
...
function get_last_day_of_month($searchFilters)
{
	...
	$CI->db->from('fy_week fw');
	$CI->db->join('financial_year f','fw.fy_id=f.fy_id');
	$CI->db->where('fw.month_no',$searchFilters['cur_month']);
	$CI->db->where('fw.year_no',$searchFilters['cur_year']);
	$CI->db->where('f.company_id',$CI->session->userdata('company'));
	$CI->db->order_by('fw.fy_week_id','desc');
	$CI->db->limit(1);
	$res=$CI->db->get();
	return $res->row_array();
}
...
/* FUNNEL REPORT START 
   Created By : Prasad
   Description : This Report explains about funnel opportunities raised in the current financial year.
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , First Bar shows how many cold,warm, hot opportunities created untill starting of financial year.
   After that, we will show month wise newly created and closed opportunity list for the current financial year.
   Last bar indicates , How many cold,warm ,hot opportunities are in open position till date.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees
   Prerequisites : Financial year should be there to view this report.
*/
function get_funnel_chart1($searchFilters)
{
	...
	$day = getOpportunityCategorizationDate();
     $hotDay = $year."-".$month."-".$day;
    $warmDate = date('Y-m-d',strtotime( $hotDay . " +1 month " ));
	/*fetching previous opportunites of before date
	  with status 1 to 5
	  hot opportunities (current month before 28 )
	  warm opportunities( after 28 and next month before 28 )
	  cold opportunities (next month after 28)
	*/  
	...
	if($searchFilters['vtime']=='m')
	{   
		//fetching week wise funnel opportunities
	    ...
	    foreach ($tline as $row) {
			   ...
			   $k2=array();
			/**
			 * Fetching funnel opportunities with open status
			 */
            ...
            // fetching closed funnel opportunites with status 6,7,8.
	...
	{   
		//fetching open opportunity list month wise
		...
		// fetching closed opportunity list month wise
    ...
    // fetching open opportunity list till date.
   ...
   // echo $CI->db->last_query();exit;
	...
	elseif($searchFilters['vtime']=='y')
	{   
		$curdate=date('Y-m-d');
		$kk=get_year_no_by_date($curdate);
		$year_num = $searchFilters['fy_dates']['year_no'];
		if($year_num==$kk['year_no'])
		{
			$timeline=date('jM-y',strtotime(date('Y-m-d')));
		}
		else
		{
			$timeline=date('jM-y',strtotime($fy_dates['end_date']));
		}
	}
	else
	{
...
/* Created By : Prasad
   ...
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report, On clicking of first bar drill down , we will get product wise opportunity list based on the selected hot or warm or cold for the given timeline.
   On clicking month wise newly created opportunities bar , we will get hot , warm , cold opportunities data.
   On Clicking month wise closed opportunities bar , we will get dropped , closed lost, closed won opportunities data
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees string , Series name
   Prerequisites : Financial year should be there to view this report.
*/
...
/* 
   ...
   In this report , we can search by users and regions.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report , On clicking of hot ,warm,cold opportunities, We will show the product wise opportunities created data.
   On clicking of dropped ,closed lost,closed won opportunities, We will show the product wise list.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees, category name , series name
   Prerequisites : Financial year should be there to view this report.
*/
function get_funnel_chart3($x_category2,$series_name2,$searchFilters)
{
    ...
    $series=array();
	$xAxisCategory=array();
	//fetching opportunity lost list by reason wise
	...
	//fetching opportunity lost list by competitor wise
...
{  
	...
	switch($timeline)
	{
		...
		case 'q':
		   ...
		   break;
	   	case "y";
			   //$fy_dates=get_previous_fiancial_year();
			  // print_r($fy_dates);die;
				// $fy_dates=get_current_fiancial_year();
				$curdate=date('Y-m-d');
				$kk=get_year_no_by_date($curdate);
				$year_no=$kk['year_no'];
				$fy_dates=get_year_start_end($date,$year_no,$searchFilters);
			break;
		default :
		     $fy_dates='';
		 break;
...
{    
	...
	$CI->db->where('fy.company_id', $CI->session->userdata('company'));
	$res=$CI->db->get();
	return $res->row_array();
}
...
function get_months_fo($start_date,$end_date)
{
	//echo $end_date;die;	
	$ci=& get_instance();
	//echo $ci->session->userdata('company');die;
	...
	{
		$ci->db->where('fw.start_date>=',$start_date);
		$ci->db->where('fw.start_date<=',$end_date);
	}
	$ci->db->group_by('fw.month_no');
	$ci->db->order_by('fw.year_no,fw.month_no');
	$res=$ci->db->get();
	$timeline=array();
...
function get_month_no_by_date($date)
{
	$ci=& get_instance();
	$ci->db->from('fy_week fw');
	$ci->db->join('financial_year fy','fw.fy_id=fy.fy_id');
	$ci->db->where('fw.start_date <=',$date);
	$ci->db->where('fw.end_date>=',$date);
	$ci->db->where('fy.company_id',$ci->session->userdata('company'));
	$res=$ci->db->get();
	return $res->row_array();
}
...
{
	$ci=& get_instance();
	$ci->db->from('custom_fy_week cfw');
	$ci->db->join('financial_year fy','cfw.fy_id=fy.fy_id');
	$ci->db->where('fy.company_id',$ci->session->userdata('company'));
	$ci->db->where('cfw.start_date <=',$date);
	$ci->db->where('cfw.end_date>=',$date);
	$res=$ci->db->get();
	return $res->row_array();
}

/* Fresh Business REPORT START 
   Created By : Prasad
   Description : This Report explains about fresh and repeat business in the current financial year.
   In this report , we can view the data by product wise and region wise.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report ,Fresh Business means customer has done the business for the first time with us.
   Repeat Business Means Customer has done the business more than 1 time with us. We can view the report in two ways .By product and By region wise.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees
   Prerequisites : Financial year should be there to view this report.
*/
...
{   
	$CI=& get_instance();
	$xAxisCategory=array();
	if($searchFilters['users']!=''&&$searchFilters['users']!='undefined')
    {   
    	//echo $searchFilters['users'];
    	$users=$searchFilters['users'];
    	$reportees=getReportingUsers($users);
	...
	if($searchFilters['measure']==1)
	{
		...
		/* Fetching all fresh cnotes with status 1 (sale orders is done)  */
		$fresh_results=$CI->Report_model->get_fresh_business_cnotes_by_product($searchFilters);
		/* Fetching all repeat cnotes with status 1 (sale orders is done)  */
	...
	if($searchFilters['measure']==2)
	{
		...
		$segment_list=array();
		$fresh_list=$repeat_list=array();
		/* Fetching all region wise fresh cnotes with status 1 (sale orders is done)  */
		$fresh_results=$CI->Report_model->get_fresh_business_cnotes_by_region($searchFilters);
		/* Fetching all region wise repeat cnotes with status 1 (sale orders is done)  */
...
/* Fresh Business REPORT 2nd level
   Created By : Prasad
   Description : This Report explains about fresh and repeat business in the current financial year.
   In this report , we can view the data by product wise and region wise.
   We can view the report by year, quarter , month ,week wise for the current financial year.
   Logic : In this report ,if we go by product wise , showing the product data for the selected segment.
   if we go by region wise , showing the sales engineers data for the selected region.
   We can view the customer data who has done the business with us.
	We can view the fresh& products data.
   Inputs: Financial year ,user_id ,region_id ,locations data,Reportees,series name,category.
   Prerequisites : Financial year should be there to view this report.
*/
function FreshBusinessChart2Data($series_name,$category,$searchFilters)
{
	...
	$searchFilters['userLocations']=$userLocations;
	$searchFilters['userProducts']=$userProducts;
	$c1Data1=array();$c1Data2=array();
    if($searchFilters['measure']==1)
    {  
    	$segment_list=$fresh_list=$repeat_list=array();
		$val=1;
		/* Fetching all product wise fresh cnotes with status 1 (sale orders is done)  */
        $fresh_results=$CI->Report_model->get_fresh_business_cnotes_by_product_customer($searchFilters,$category,$val);
		$val=2;
		/* Fetching all product wise repeat cnotes with status 1 (sale orders is done)  */
		...
		/* Fetching all customer wise fresh cnotes with status 1 (sale orders is done)  */
	...
	else
	{
	    ...
	    $tline=get_year_based_timeline($searchFilters,$rr);
	     foreach ($tline as $row) {
			...
			//getting funnel opportunities
			$yt_timeline=date('M-y',strtotime($row['timeline']));
			//fetching funnel opportunites with status 1,2,3,4,5
			$res = $CI->Report_model->get_run_rate_funnel_opportunites($searchFilters,$row);
			if(count($res)>0)
			   	 { 
				   	$k1['opened_value']=@$res['opened_value'];
...
function incentives_user_chart1($searchFilters)
{
	...
	foreach($product_category as $row1)
	{
		...
		$incentives=$CI->Common_model->get_data_row('incentives',array('role_id'=>$role_id,'fy_id'=>$searchFilters['year'],'status'=>1,'company_id'=>$CI->session->userdata('company')));
...
function get_fy_year_array($fy_dates)
{
	$fy_year=get_current_fiancial_year();
	// echo "<pre>";print_r($fy_year);die;
	$ci=& get_instance();
	// $ci->db->select('fw.*,year_no as t');
	$ci->db->select('f.*');
	$ci->db->from('financial_year f');
	$ci->db->join('fy_week fw','f.fy_id=fw.fy_id');
	$ci->db->where('f.company_id',$ci->session->userdata('company'));
	...
	{   
		...
		// $ydate = explode('-',$fy_dates['name']);
		// // echo "<pre>";print_r($ydate[0]);die;
		$selected='';
		$selected=($row['name']==$row['name'])?'selected':'';
		$qry_data.='<option value="'.$row['name'].'"'.$selected.'>'.$row['name'].'</option>';
		
	}
	return $qry_data;
}

function get_year_no_by_date($date)
{
	$ci=& get_instance();
	$ci->db->from('fy_week fw');
	$ci->db->join('financial_year fy','fw.fy_id=fy.fy_id');
...
function get_year_start_end($date,$year_no,$searchFilters='')
{   
	//print_r($searchFilters);
	if(@$searchFilters['duration']!='')
	{
	...
	$CI=& get_instance();
	$curdate=date('Y-m-d');
	$CI->db->select('min(fws.start_date) as start_date,max(fws.end_date) as end_date,fws.year_no');
	$CI->db->from('financial_year fy');
	$CI->db->join('fy_week fws','fy.fy_id=fws.fy_id');
    $CI->db->where('fy.company_id',$CI->session->userdata('company'));
	$CI->db->where('fws.year_no',$year_no);
	// $CI->db->where('fy.start_date<=',$curdate);
	// $CI->db->where('fy.end_date>=',$curdate);
	$CI->db->group_by('fws.year_no');
	$res=$CI->db->get();
	return $res->row_array();
}
...
class New_outstanding_format extends Base_controller {
    ...
    */
    public function new_so_amount_upload()
    {
        ...
        $data['countries'] = $this->Common_model->get_data('location',array('territory_level_id'=>3,'status'=>1));
    ...
    {  
        ...
        //Fetching regions
        $regions = $this->New_outstanding_format_m->get_regions($country_id);
   ...
   public function insert_new_so_amount_upload()
    {
        ...
        {  
            ...
            for($i=2;$i<=$totalrows;$i++)
            {
                ...
                {   
                    $r_id = $this->New_outstanding_format_m->get_region_id($region,$country_id);
                    $region_id = $r_id['location_id'];
                    ...
                    if($updated_count <= 0)
                    {
                        ...
                        {
                            $missing++;
                            $remarks_missing_text.=$region.' does not Exists,';
                        }
                    }
                    else
                    {
            ...
            */            
             ...
             if(count($searchResults)>0)
            {
                ...
                {      
                        ...
                        $exceldata[] = getUserName(@$row['created_by']);

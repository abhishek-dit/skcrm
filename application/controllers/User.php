...
class User extends Base_controller {
	...
	public function Users() {
        ...
        $data['company'] = $this->Common_model->get_data('distributor_details',array('1'=> 1));
	...
	public function addUser()
	{
		...
		$data['branch'] = array(''=>'Select Branch') + $this->Common_model->get_dropdown('branch', 'branch_id', 'name', ['company_id'=>$data['sess_company']]);
	...
	public function insertUser()
	{
			...
			case 4: // Sales Engineer (SE)
					...
					if(!$state) //  if no states existed
					{
						...
						if(!$district) { // if no districts existed
							...
							$district_parent_str = $this->input->post('district_parent',TRUE);
							$district_explode = explode("p",$district_parent_str);
							$district_parent=[];
							foreach($district_explode as $row1)
							{
								$arr1 = explode("k",$row1);
								$district_parent[$arr1[0]] = $arr1[1];
							}
							...
							if($city) { // if cities existed
								$city_parent_str = $this->input->post('city_parent',TRUE);
								$city_explode = explode("p",$city_parent_str);
								$city_parent=[];
								foreach($city_explode as $row)
								{
									$arr = explode("k",$row);
									$city_parent[$arr[0]]=$arr[1];
								}
		...
		switch($role_id) {
			...
			case 6: // RSM
			case 8: // NSM
				$product_str = $this->input->post('product',TRUE);
				$product = explode("p",$product_str);
				if($product)
	...
	public function deleteUser($encoded_id)
	{
		//echo 'hi';
			...
			$check = $this->Loginmodel->get_last_punch_in($user_id);
			...
			$this->session->set_flashdata('response','<div class="alert alert-success alert-white rounded"><button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button><div class="icon"><i class="fa fa-check"></i></div><strong>Success!</strong> User has been De-Activated successfully!</div>');
			redirect(SITE_URL.'users');
	...
	public function editUserDetails($encoded_id)
	{
		...
		$data['role_level_id'] = $role[0]['role_level_id'];
		$data['isd'] = $this->Common_model->get_dropdown('isd', 'isd', 'isd', []);
		$data['branch'] = array(''=>'Select Branch') + $this->Common_model->get_dropdown('branch', 'branch_id', 'name', ['company_id'=>$this->session->userdata('company')]);
	...
	public function editUserProducts($encoded_id)
	{
		...
		// GET USER PRODUCTS WITH ID,NAME
				...
				$data['all_productCategories'] = $this->Common_model->get_data('product_category',array('status'=>1,'company_id'=>$this->session->userdata('company')));
	...
	public function updateUserProducts(){
		...
		$user_products = array_filter($user_products);
		if(count($user_products)>0){
			//print_r(count($user_products));die;

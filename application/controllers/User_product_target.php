...
class User_product_target extends Base_controller {
	...
	*/
	...
	public function bulk_upload_weekly_user_product_targets($encoded_id)
	{
		...
		$data['fy_id'] = $this->Common_model->get_value('financial_year',array("start_date<="=>$curdate,"end_date>="=>$curdate,'status'=>1,'company_id'=>$this->session->userdata('company')),'fy_id');
	...
	public function download_weekly_user_product_target_csv()
	{
		...
		//$data['user_product_targerts'] = $user_product_targerts;
			...
			{
				$data.='<tr><td colspan="55" align="center">No Products Assigned</td></tr>';
			}
			$data.='</tbody>';
			$data.='</table>';

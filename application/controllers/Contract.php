...
class Contract extends Base_controller {
    ...
    #Fetching Cnote Details which are in open status
	public function openCNoteDetails($encoded_id)
	{
        ...
        $cNoteDetails = array();
        /* 
        Fetching Cnote details which are waiting for RBH approval and SO Entry
        */
    ...
    #Fetching CNotes with closed status
    public function closedCNoteDetails($encoded_id)
    {
        ...
        $search_fields = 0;
        $cNoteDetails = array();
        #Fetching all Cnotes whose so entry is done 
        $cNoteDetails = $this->Contract_model->getCNoteDetails($lead_id);
    ...
    {   
        if ($this->input->post('submitCNote') != "") 
        {
            ...
            /*
            Selected quotes should have same billing through
            then only cnote will be generated 
            otherwise redirecting to cnote page
            */
            if($count_type==0)
            {
                //echo 123;die;  
                $this->session->set_flashdata('response','<div class="alert alert-danger alert-white rounded">
                                        ...
                                        <strong>Message!</strong> Please make sure that selected quotes should have same Billing through.
                                     </div>');
            ...
            /*
              selected quotes should have same currrency
              otherwise we cannot generate cnote
            */
            if($currency_count == 0)
            {
                $this->session->set_flashdata('response','<div class="alert alert-danger alert-white rounded">
                                        <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
                                        <div class="icon"><i class="fa fa-times"></i></div>
                                        <strong>Error!</strong>Please make sure that selected quotes should have same Currency.
                                     </div>');
                redirect($this->agent->referrer());

            }
            //print_r(count($quote_id));die;
            if(count($quote_id)>1)
            {
                //echo 123;die;  
                ...
                /*
                payment terms (Warranty, Advance, Balance Payment Days, Dealer Commision, Dealer) to be same for all quotes in a contract note.
                with different payment terms ,cnote generation is not possible.
                */
                if($hasError)
                {
                    //exit('error');
...
/*
Deletion of Cnote.
Reverting the status of lead , opportunity, quote to previous status
*/
public function deleteContractNote($encoded_id)
    {
        $data1 = explode('&',$encoded_id);
        $contract_note_id1 = $data1[0];
        $cnote_num1 = $data1[1];
            $contract_note_id=@icrm_decode($contract_note_id1);
            $where = array('contract_note_id' => $contract_note_id);
            $c_lead_id=$this->Contract_model->get_lead_id($contract_note_id);
            $cnote_num=@icrm_decode($cnote_num1);
            // print_r($cnote_num); exit;
            ...
            else
            {
                ...
                $this->session->set_flashdata('response','<div class="alert alert-success alert-white rounded">
                                    <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
                                    <div class="icon"><i class="fa fa-check"></i></div>
                                    <strong>Success!</strong> Contract Note '.$cnote_num.' has been Deleted successfully!
                                 </div>');
            }
            redirect(SITE_URL.'manageContractNotes');

    }
    #Fetching all cnotes if RBH approval process is done
    ...
    #Fetching all cnotes after so entry is done
    ...
    #Fetching cnotes after rbh approval process is completed
    ...
    # Upload : SO entry for all Cnotes
         ...
         if($_FILES["uploadCsv"]["size"] > 0)
         {
            ...
            if($res){
                foreach ($res->result_array() as $cnote_row) {
                    $user_cids[] = $cnote_row['cnote_number'];
                }
            }
             while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE)
             {
                ...
                if($so_entry_num != '')
                {
                    ...
                    if(in_array($contract_note_id, $user_cids)||$this->session->userdata('role_id') == 2||$this->session->userdata('role_id') == 14){
                        $cnote_id = $this->Common_model->get_value('contract_note',array('cnote_number'=>$contract_note_id),'contract_note_id');
                        $data = array('SO_number'=>$so_entry_num,'status'=>2,'modified_by'=>$this->session->userdata('user_id'),'modified_time'=>date('Y-m-d H:i:s'));
                        $where = array('contract_note_id'=>$cnote_id, 'status' => 1);
                        //update so number

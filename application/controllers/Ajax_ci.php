...
class Ajax_ci extends Base_controller {
	function __construct(){
		...
		$this->load->model('Customer_category_model','',TRUE);
	}//__construct()
	...
	function getDistrictsByState(){
		...
		$locations = $this->Ajax_m->getLocationsByParent($stateId);
			...
			foreach($locations as $location) {
				...
				$data .= '<input type="hidden" class="district_parent" value="'.$location['location_id'].'k'.$stateId.'">';
			}
		}
		...
		foreach($sres->result_array() as $state)
		{
			...
			$data .= '<h6>'.$district['location'].'</h6>';
				...
				foreach($locations as $location) {
					...
					$data .= '<input type="hidden" class="city_parent" value="'.$location['location_id'].'k'.$district['location_id'].'">';
				}
			}
		}
		...
		foreach($productCategories as $productCatId) {
			...
			foreach($pg_res->result_array() as $productGroup) {
				...
				foreach($p_res->result_array() as $product) {
					//$chkd = (is_array($current_districts)&&in_array($location['location_id'],$current_districts))?'checked':'';
					$data .= '<div class="radio"><label> <input type="checkbox"  value="'.$product['product_id'].'" checked class="icheck product pr prodGroup'.$productGroup['group_id'].' prodCat'.$productCatId.'"> '.$product['description'].'</label> </div>';
				}
			}
		}
  ...
  #Fetching Inactive User Open Lead Locations
	...
	* return: 1/0(boolean)
	**/
  	function is_customerNameExist(){
		$customer_name = $this->input->post('customer_name');
		$customer_id = icrm_decode($this->input->post('customer_id'));
		$data = '';
		echo $this->Ajax_m->is_customerNameExist($customer_name,$customer_id);
  	}
  /**
	* check Speciality Name exist or not
	* author: mahesh , created on: 19th july  2016 02:00 AM, updated on: --
	* @param: $speciality name(string)
	**/   
    function is_specialityNameExist()
    {
		...
		/**
	* check Branch Name exist or not
	* author: mahesh , created on: 19th july  2016 02:00 AM, updated on: --
	* @param: $branch name(string)
	**/  
  	function is_branchNameExist()
    {
        $branch_name = $this->input->post('branch_name');
        $branch_id = icrm_decode($this->input->post('branch_id'));
		    $data = '';
		    echo $this->Branch_model->is_branchNameExist($branch_name,$branch_id);
  	}
	...
	**/  
  	function is_categoryNameExist()
    {
      	$category_name = $this->input->post('category_name');
        $category_id = icrm_decode($this->input->post('category_id'));
				$data = '';
				echo $this->Customer_category_model->is_categoryNameExist($category_name,$category_id);
  	}

		/**
		* check subcategory Name exist or not
		* author: mahesh , created on: 19th july  2016 02:00 AM, updated on: --
		* @param: $category name(string)
		* @param: $sub_category id(int)
		**/  
  	function is_subcategoryNameExist()
    {
        $sub_category_name = $this->input->post('category_name');
        $sub_category_id = icrm_decode($this->input->post('sub_category_id'));
        $category_id = $this->input->post('category_id');
				$data = '';
				echo $this->Customer_category_model->is_subcategoryNameExist($sub_category_name,$sub_category_id,$category_id);
		}
		/**
		* Fetching Branches based on geo
		* author: mahesh , created on: 19th july  2016 02:00 AM, updated on: --
		**/  
    function getbranch_CountriesByGeo()
  	{
  		$locationParentId = $this->input->post('locationParentId');
		$data = '';
		$locations = $this->Ajax_m->getLocationsByParent($locationParentId);
		...
		/**
		* Fetching branches based on region
		* author: mahesh , created on: 19th july  2016 02:00 AM, updated on: --
		**/  
  	function getbranch_RegionsByCountry()
  	{
  		$locationParentId = $this->input->post('locationParentId');
		$data = '';
		$locations = $this->Ajax_m->getLocationsByParent($locationParentId);
		...
		/**
		* check Channel Partner Name exist or not
		* author: prasad , created on: 19th mar  2019 02:00 PM, updated on: --
		* @param: $channel_partner name(string)
		**/  
  	...
  	function is_companyNameExist(){
		$company_name = $this->input->post('company_name');
		$company_id = $this->input->post('company_id');
		$data = '';
		echo $this->Ajax_m->is_companyNameExist($company_name,$company_id);
  	}

...
class MarginAnalysis extends Base_controller {
    ...
    function margin_analysis_approval_list(){
        ...
        //echo print_r($data['quoteSearch']); exit;
    ...
    public function submitMarginAnalysisApproval()
    {
        ...
        if($action!='')
        {
            ...
            $opp_number = $this->Common_model->get_value('opportunity',array('opportunity_id'=>$opportunity_id),'opp_number');
                ...
                case 1: // If approved
                    ...
                    if($cond)//finl approval
                    {
                        ...
                        // echo 'completed_op_count'.$completed_op_count ; die;
                                ...
                                //$quote_revision_id1 = $this->Common_model->get_value("quote_revision", array('quote_revision_id' => $quote_revision_id), "quote_revision_id");
                                ...
                                $quote_revision_id1 = $this->Common_model->get_value("quote_revision", array('quote_revision_id' => $quote_revision_id), "quote_revision_id");
                                $lead_user = $this->Common_model->get_data_row('user',array('user_id'=>$lead_owner));
                                //$quote_reference_id1 = getQuoteRevisionReferenceID($lead_id,$quote_id,$quote_revision_id1,$quote_number);
                                $quote_reference_id1 = getQuoteRevisionReferenceID($lead_id,$quote_id,$quote_revision_id1,$quote_number);
                                //print_r($quote_revision_id1);die;
                                ...
                                $cc_allRolesEmail = getAllRolesEmailID($cc_users[0]['branch_id']);
                                ...
                                // if($cc_allRolesEmail)
                                // {
                                //     foreach ($cc_allRolesEmail as $cc_row) {
                                //         $cc[] = $cc_row['email_id'];
                                //     }
                                //     $cc = array_unique($cc);
                                // }
                                $cc = (count($cc)>0)?implode(',', $cc):'';
                                $cc1 = $cc . "," . "CRM@skanray.com";
                                $to = @$lead_user['email_id'];
                                // $subject = 'Opportunity: '.$opp_number.' has been approved with Quote Reference ID: '.$quote_reference_id1;
                                // $message = '<p>Hi '.$lead_user['first_name'].' '.$lead_user['last_name'].',</p>';
                                // $message .= '<p>Opportunity: '.$opp_number.' has been successfully pass through Margin Analysis </p>';
                                // //$message .= '<p>hello </p>';
                                // $message .= '<p><strong>Quote Reference ID:</strong> '.$quote_reference_id1.' </p>';
                                // $message .= '<p><strong>Opportunity Details:</strong> '.$opportunity_details.' </p>';


                                $subject = 'Quote Reference ID: '.$quote_reference_id1.' has been approved';
                                $message = '<p>Hi '.$lead_user['first_name'].' '.$lead_user['last_name'].',</p>';
                                ...
                                //$message .= '<p>user'.$user_id.'</p>';
                                //$message .= '<p>Note'.$user_id.'</p>';
                                $message .= '<p>Regards,</p>';
                            ...
                            else
                            {
                                ...
                                $lead_user = $this->Common_model->get_data_row('user',array('user_id'=>$lead_owner));
                                $to = @$lead_user['email_id'];
                                $subject = 'Opportunity: '.$opp_number.' has been approved with Quote Reference ID: '.$quote_reference_id ;
                                $message = '<p>Hi '.$lead_user['first_name'].' '.$lead_user['last_name'].',</p>';
                                //$message .= '<p>hello1</p>';
                    ...
                    else
                    {
                        ...
                        $quote_reference_id1 =getQuoteRevisionReferenceID($lead_id,$quote_id,$quote_revision_id1,$quote_number);
                        $opportunity_details = getOpportunityDetails($opportunity_id);
                        $role = getRoleShortName($next_approval_role);
                        // $margin_approval_id = $this->Common_model->get_value("quote_op_margin_approval", array('quote_revision_id' => $quote_revision_id1), "margin_approval_id");
                        $approved_by = getApprovalRole($margin_approval_id);
                        $approved_by_role = getRoleShortName($approved_by);
                        $cc_users = getMarginAnalysisCcUsersList($role_id,$margin_approval_id);
                        $cc = array();
                        if($cc_users)
                        {
                        ...
                        $to = @$lead_user['email_id'];
                        if($conditionApproval[0]['condition'] == 1){
                            $subject = 'Quotation: '.$quote_reference_id1.' has been approved';
                        }
                        else{
                             $subject = 'Opportunity: '.$opp_number.' has been approved with Quote Reference ID: '.$quote_reference_id1;
                        }
                        $message = '<p>Hi '.$lead_user['first_name'].' '.$lead_user['last_name'].',</p>';
                        //$message .= '<p>hello '.$role.'</p>';
                        if($conditionApproval[0]['condition'] == 1){
                            $message .= '<p> Quotation has been approved by '.$approved_by_role.' and needs approval from '.$role.'</p>';
                        }else{
                            $message .= '<p> Opportunity has been approved by '.$approved_by_role.' and needs approval from '.$role.'</p>';
                        }
                        $message .= '<p><strong>Quote Reference ID:</strong> '.$quote_reference_id1.' </p>';
                        if($conditionApproval[0]['condition'] == 1){
                      ...
                      // print_r($next_approval_role);die;
                        if(count($quote_approvers)>0)
                        {
                           // echo 123;die;
                            foreach($quote_approvers as $key =>$urow) {
                                $to = $urow['email_id'];
                                //print_r($to);die;
                                $cc = "crm@entransys.com";
                    ...
                    if($conditionApproval[0]['condition'] == 1){
                        ...
                        $success_msg = 'Opportunity : '.$opp_number.' quote revision has been approved successfully';
                    }
                break;
                
               case 2: // If Rejected
               $success_msg = 'Opportunity : '.$opp_number.' quote revision has been rejected successfully';
                    // Check if all opportunities are approved.
                    // Get opportunities count By quote_revision_id
                    $op_count = getMarginAnalysisOpportunityCountByQuoteRevision($quote_revision_id);
                    $completed_op_count = getCompletedMarginAnalysisOpportunityCountByQuoteRevision($quote_revision_id);
                    ...
                    // Email Notification to lead owner
                        ...
                        //print_r($quote_reference_id);die;
                        $quote_number = $this->Common_model->get_value("quote", array('quote_id' => $quote_id), "quote_number");
                        $quote_revision_id1 = $this->Common_model->get_value("quote_revision", array('quote_revision_id' => $quote_revision_id), "quote_revision_id");
                        //$lead_user = $this->Common_model->get_data_row('user',array('user_id'=>$lead_owner));
                        $quote_reference_id1 = getQuoteRevisionReferenceID($lead_id,$quote_id,$quote_revision_id1,$quote_number);
                        //print_r($quote_reference_id);die;
                        $rejected_user_name = $this->Common_model->get_value("user", array('user_id' => $user_id), "first_name");
                        $rejected_emp_id = $this->Common_model->get_value("user", array('user_id' => $user_id), "employee_id");
                        $reject_user_role_id = getRoleShortName($role_id);
                        $cc = (count($cc)>0)?implode(',', $cc):'';
                        $cc1 = $cc . "," . "CRM@skanray.com";
                        $to = @$lead_user['email_id'];
                        $subject = 'Opportunity - '.$opp_number.' has been rejected for Quote Reference ID: '.$quote_reference_id1;
                        $message = '<p>Hi '.$lead_user['first_name'].' '.$lead_user['last_name'].',</p>';
                        $message .= '<p>Opportunity: '.$opp_number.' has been failed to pass through Margin Analysis </p>';
                        $message .= '<p><strong>Quote Reference ID:</strong> '.$quote_reference_id1.' </p>';
                        $message .= '<p><strong>Opportunity Details:</strong> '.$opportunity_details.' </p>';
                        $message .= '<p><strong>Rejected By:</strong>'.$reject_user_role_id.'('.$rejected_emp_id.'-'.$rejected_user_name.')</p>';
                        $message .= '<p><strong>Reason for rejection:</strong>'.$remarks.'</p>';
                        $message .= '<p>Regards,</p>';
                        $message .= '<p>iCRM,<br>SkanRay</p>';
                        /*echo 'To:'.$to.'<br>'.'CC:'.$cc.'<br>Subject:'.$subject.'<br>Message:'.$message;
                        exit;*/
    ...
    public function marginAnalysisConfig()
    {
        ...
        $data['marginApproval'] = $this->Common_model->get_data('quote_approval_config',array('status'=>1,'company_id'=>$_SESSION['company']));
    ...
    public function submitPoApproval()
    {
        ...
        if($action!='')
        {
            ...
            $po_number = $this->Common_model->get_value('purchase_order',array('purchase_order_id'=>$purchase_order_id),'po_number');
                ...
                case 1: // If approved
                    ...
                    if($role_id==$ma_row['close_at'])
                    {
                        ...
                        if($product_count==$completed_product_count)
                        {
                            ...
                            $cn_data['cnote_counter']=$cnote_counter;
                            $cn_data['cnote_number']=$cnote_number;
                            $cn_data['company_id']= $this->session->userdata('company');

...
class Report2 extends Base_controller {
	...
	/* MARGIN ANALYSIS REPORT START 
	Created By : Mahesh
	Description : This Report explains about Product wise Margin after successful generation of contract note earned by company .
	In this report , we can search by users,regions,product,segment,customer,dealer,from date ,to date.
	We can download the margin analysis report based on the given search filters
	Logic : In this report , We are showing data in the form of tabular format. In the first level, we are calculating the margin for the segment wise , We can go into product
	level details if we click on the segment wise margin.In the tabular format , we are showing the revenue generated in lacs and gross margin .
	Formulae : To calculate the order amount , ROUND(SUM( CASE WHEN ma.margin_approval_id IS NULL THEN (o.required_quantity*qd.total_value)*(1-qr.discount/100) ELSE ( CASE WHEN ma.discount_type = 1 THEN (o.required_quantity*qd.total_value)*(1-ma.discount/100) ELSE (o.required_quantity*qd.total_value)- (ma.discount*qd.currency_factor) END ) END )) AS order_value
	Inputs: user_id ,region_id ,locations data,Reportees.
	Prerequisites : Valid User Id And password.
	*/
	...
	{	
        ...
        {
			$data['regions'] = $this->Report_model->get_user_location_regions();
			...
			//fetching margin details for each product
	...
	Created By : Mahesh
	Description : This Report explains about CNOTE wise Margin after successful generation of contract note earned by company .
	In this report , we can search by users,regions,product,segment,customer,dealer,from date ,to date.
	We can download the cnote margin analysis report based on the given search filters
	Logic : In this report , We are showing data in the form of tabular format.In the tabular format , we are showing the SO Number along with product details .
	Formulae : To calculate the order amount , ROUND(SUM( CASE WHEN ma.margin_approval_id IS NULL THEN (o.required_quantity*qd.total_value)*(1-qr.discount/100) ELSE ( CASE WHEN ma.discount_type = 1 THEN (o.required_quantity*qd.total_value)*(1-ma.discount/100) ELSE (o.required_quantity*qd.total_value)- (ma.discount*qd.currency_factor) END ) END )) AS order_value
	Inputs: user_id ,region_id ,locations data,Reportees.
	Prerequisites : Valid User Id And password.
	*/
	...
	// 		// echo "<pre>";print_r($cnote_results);die;
	// 		//echo '<pre>'; print_r($segment_margin_data); exit;
	// 		$this->load->library('excel');
    //         $this->excel->setActiveSheetIndex(0);
    //         $this->excel->getActiveSheet()->setTitle('CNote Margin Report');
	// 		$this->excel->getActiveSheet()->setCellValue('A1', 'C-Note ID');
	// 		$this->excel->getActiveSheet()->setCellValue('B1', 'Lead ID');
    //         $this->excel->getActiveSheet()->setCellValue('C1', 'Type');
    //         $this->excel->getActiveSheet()->setCellValue('D1', 'Opportunity ID');
    //         $this->excel->getActiveSheet()->setCellValue('E1', 'Product Code');
    //         $this->excel->getActiveSheet()->setCellValue('F1', 'Description');
    //         $this->excel->getActiveSheet()->setCellValue('G1', 'Qty');
    //         $this->excel->getActiveSheet()->setCellValue('H1', 'Warranty (Months)');
    //         $this->excel->getActiveSheet()->setCellValue('I1', 'Advance (%)');
    //         $this->excel->getActiveSheet()->setCellValue('J1', 'Balance Payment Days');
    //         $this->excel->getActiveSheet()->setCellValue('K1', 'Dealer Commission (%)');
    //         $this->excel->getActiveSheet()->setCellValue('L1', 'Dealer');
	// 		$this->excel->getActiveSheet()->setCellValue('M1', 'Highest Approver');
	// 		$this->excel->getActiveSheet()->setCellValue('N1', 'MRP');
    //         $this->excel->getActiveSheet()->setCellValue('O1', 'Order Value');
    //         $this->excel->getActiveSheet()->setCellValue('P1', 'Net Selling Price');
    //         $this->excel->getActiveSheet()->setCellValue('Q1', 'Basic Price');
    //         $this->excel->getActiveSheet()->setCellValue('R1', 'Discount %');
    //         $this->excel->getActiveSheet()->setCellValue('S1', 'Cost of Warranty');
    //         $this->excel->getActiveSheet()->setCellValue('T1', 'Cost of Finance');
    //         $this->excel->getActiveSheet()->setCellValue('U1', 'Commission to Dealer in Rs');
    //         $this->excel->getActiveSheet()->setCellValue('V1', 'Cost of Free Supply');
    //         $this->excel->getActiveSheet()->setCellValue('W1', 'Gross Margin %');
    //         $this->excel->getActiveSheet()->setCellValue('X1', 'Gross Margin in Rs');
    //         $this->excel->getActiveSheet()->setCellValue('Y1', 'Net Margin %');
    //         $this->excel->getActiveSheet()->setCellValue('Z1', 'Net Margin in Rs');
    //         $this->excel->getActiveSheet()->setCellValue('AA1', 'Customer');
    //         $this->excel->getActiveSheet()->setCellValue('AB1', 'SO Number');
    //         $this->excel->getActiveSheet()->setCellValue('AC1', 'Region');
    //         $this->excel->getActiveSheet()->setCellValue('AD1', 'CNote Created By');
    //         $this->excel->getActiveSheet()->setCellValue('AE1', 'CNote Created User Role');
    //         $this->excel->getActiveSheet()->setCellValue('AF1', 'Order Status');
    //         $this->excel->getActiveSheet()->setCellValue('AG1', 'Invoice Cleared By');
    //         $this->excel->getActiveSheet()->setCellValue('AH1', 'CNote Created Time');
    //         $exceldata="";
	...
	// 					$warranty1 = @$parr[4];
	// 					$type1 = @$parr[5];
	// 					$cnote_type = ($type1==1)?'Regular':'Purchase Order';

	// 					$advance1 = @$parr[6];
	// 					$advance = @$parr[7];
	// 					$balance_payment_days = @$parr[8];
	// 					$dealer_commission = @$parr[9];
	// 					$distributor_name = @$parr[10];
	// 					$mrp = @$parr[11];

	// 					if($advance1==2) // Advance in Rs
	// 					{
	// 						$advance = round($advance1*100/$row['order_value'],2);
	// 					}
	// 					$data['advance'] = $advance;
	// 					$data['balance_payment_days'] = ($balance_payment_days>0)?$balance_payment_days:0;
	// 					$data['dealer_commission'] = $balance_payment_days;

					
	// 					$ma_data = marginAnalysis($data);
	//                     $exceldata=array();
	// 					$exceldata[] = @$row['cnote_id'];
	// 					$exceldata[] = @$row['lead_id'];
	//                     // if($k==0)
	//                     // {
	...
	// 					$exceldata[] = @$warranty1;
	// 					$exceldata[] = @$advance;
	// 					$exceldata[] = @$balance_payment_days;
	// 					$exceldata[] = (@$dealer_commission>0)?@$dealer_commission:'';
	// 					$exceldata[] = @$distributor_name;

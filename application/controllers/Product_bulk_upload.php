    ...
    */
    public function downloadProduct_Upload()
    {
        if($this->input->post('downloadProduct')!='') 
        {
            ...
            $this->excel->getActiveSheet()->setCellValue('N1', 'Technical Specifications');
            $this->excel->getActiveSheet()->setCellValue('O1', 'Availability ('.$note.')');
            $exceldata="";
            if(count($products)>0)
            {
                $arr=array();
        ...
        {  
          ...
          //  $objReader= PHPExcel_IOFactory::createReader('Excel2007'); // For excel 2007   
            ...
            for($i=2;$i<=$totalrows;$i++)
            {
                ...
                $features= $objWorksheet->getCellByColumnAndRow(13,$i)->getValue();
                $availability= $objWorksheet->getCellByColumnAndRow(14,$i)->getValue(); //Excel Column 15
                ...
                $company_id = $this->session->userdata('company');
                $currency_id = $this->Common_model->get_value('company',array('company_id'=>$company_id),'currency_id');
               if($product_code!=''&&$segment!=''&&$product_type!='' &&$mrp_price !==''&&$freight_insurance!==''&&$gst!==''&&$rrp!==''&&$dp!==''&& (strtoupper($availability)=='ACTIVE' || strtoupper($availability)=='INACTIVE') )
                {   
                    $products = $this->Common_model->get_data_row('product',array('name'=>$product_code,'company_id'=>$company_id));
                    ...
                    {   
                        ...
                        else
                        {
                            ...
                            if($updated_count > 0)
                            {
                                 ...
                                 $this->Common_model->update_data('product_price_history',$update_data,array('product_id'=>$products['product_id'],'start_date'=>$as_on_date,'currency_id'=>$currency_id,'company_id'=>$company_id));
                            }
                            else
                            {

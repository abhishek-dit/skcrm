...
class Location extends Base_controller {
	...
	public function locationAdd()
	{
		if($this->input->post('submitLocation') != "")
		{
			...
			if($location_id == "")
			{
				...
				if($territory_level_id==5)
				{
					$dataArr['tag']=$this->input->post('tag');
				}
				if($territory_level_id==3)
				{
					$dataArr['currency_id']=$this->input->post('currency_id');
				}
	...
	public function downloadState()
	{
		if($this->input->post('downloadState')!='') {
			...
			$header = '';
			$data ='';
			$titles = array('S.NO','Country Name','Region Name','State Name','Short Name','Created Time','Modified By','Modified Time');
			$data = '<table border="1">';
			$data.='<thead>';
			$data.='<tr>';
			foreach ( $titles as $title)

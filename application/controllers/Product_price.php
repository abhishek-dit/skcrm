...
class Product_price extends Base_controller {
    ...
    */
    public function product_price_upload()
    {
        ...
        $data['nestedView']['cur_page'] = 'product_price_upload';
        $data['nestedView']['parent_page'] = 'product_price_upload';
    ...
    {  
        ...
        //Fetching products
        $product_results = $this->Common_model->get_data('product',array('status'=>1,'company_id'=>$this->session->userdata('company')));
    ...
    public function insert_product_price_upload()
    {
        ...
        {  
            ...
            $default_currency_id = $this->Common_model->get_value('company',array('company_id'=>$company_id),'currency_id');
            $this->load->library('excel');
           ...
           //  $objReader= PHPExcel_IOFactory::createReader('Excel2007'); // For excel 2007   
            ...
            for($i=2;$i<=$totalrows;$i++)
            {
               ...
               if(($product_name!='') &&($mrp_price !=''||$base_price!=''|| $freight_insurance!=''||$gst!=''||$rrp!=''||$dp!='' ))
                {   
                    $products = $this->Common_model->get_data_row('product',array('name'=>$product_name,'company_id'=>$company_id));
                    $query = 'SELECT * from product_price_history WHERE product_id="'.$products['product_id'].'" AND start_date="'.$as_on_date.'" AND currency_id="'.$currency_id.'" AND company_id="'.$company_id.'" ';
                    ...
                    {   
                        ...
                        {   
                            ...
                            $latest_record=$this->Product_price_m->get_latest_price_record($products['product_id'],$currency_id,$company_id);
                            ...
                            {
                                $cur_qry='select * from product_currency where product_id="'.$products['product_id'].'" AND currency_id="'.$currency_id.'" AND company_id="'.$company_id.'" ';
                                $cur_count = $this->Common_model->get_no_of_rows($cur_qry);
                                if($cur_count>0)
                                {   
                                    //updating the product data in product currency table
                                    $this->Common_model->update_data('product_currency',$update_data,array('product_id'=>$products['product_id'],'currency_id'=>$currency_id,'company_id'=>$company_id));
                                    $updated_files++;
                                }
                                else
                                {   
                                     //Inserting the product data in product currency table
                                    unset($dat4['upload_id']);
                                    unset($dat4['start_date']);
                                    $this->Common_model->insert_data('product_currency',$dat4);
                                    $inserted_files++;
                                }
                            }
                    ...
                    else
                    {
                        ...
                        if($currency_id==$default_currency_id)
                        {
                            $this->Common_model->update_data('product',$update_data1,array('product_id'=>$products['product_id'],'company_id'=>$company_id));
                        }
                        else
                        {
                            $this->Common_model->update_data('product_currency',$update_data1,array('product_id'=>$products['product_id'],'company_id'=>$company_id,'currency_id'=>$currency_id));
                        }
                        $this->Common_model->update_data('product_price_history',$update_data,array('product_id'=>$products['product_id'],'company_id'=>$company_id,'currency_id'=>$currency_id,'start_date'=>$as_on_date));

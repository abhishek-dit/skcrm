        ...
        <div class="content">
          <div class="row no-gutter" >
            <div class="col-sm-12">
              ...
              <div class="col-sm-2">
                <input type="number" name="" placeholder="In Seconds" class="form-control" id="time">
              </div>
              <div class="col-sm-2">
                <button type="submit" id="time_btn" value="1" class="btn btn-success"><i class="fa fa-search"></i> </button>
...
/*var latlongs= [
       {lat:16.601115, long:79.121286},
       {lat:24.426224, long:88.388284},
       {lat:22.733206, long:71.939093},
       {lat:17.413920, long:73.464637},
       {lat:23.168344, long:80.134033},
       {lat:28.642500, long:77.205203},
       {lat:12.885667, long:87.447622},
       {lat:12.824634, long:76.375071},
       {lat:10.905187, long:76.020087}
     ];*/
var latlongs = <?php echo json_encode($lat_regions); ?>;
...
function setInter(){
    ...
    intervalId = setInterval(function(){
     ...
     var center = new google.maps.LatLng(latlongs[i].lat, latlongs[i].long);
           map.setCenter(center);
           map.panTo(center);
          map.setZoom(10);
...
function addAllMarker(SalesEngg){
    ...
    var ASSET_URL = '<?php echo assets_url()?>';
       ...
       for (let i = 0; i < SalesEngg.length; i++) {

         console.log('dgdddv',SalesEngg[i]);
        if(SalesEngg[i].start_time !== null &&  SalesEngg[i].end_time === null) {
          var image = {
...
//        }
       ...
       function updateMarker(message){
         console.log(" message from updateMarker");
         console.log(message);
        var ASSET_URL = '<?php echo assets_url()?>';
        ...
        if(ll[2] === 'left' ){
          console.log('left from updateMarker')
          // var image = {
          //         url: ASSET_URL+'images/offline.svg',
          //         size: new google.maps.Size(20, 32),
          //       };
          ...
          SalesEngg.forEach(function(marker) {
            console.log("marker");
            console.log(marker);
                     if(marker.status === "2") {
                  //  if(marker.start_time !== null &&  marker.end_time === null) {
...
<script>
    var socket;
  //socket = io('https://powerful-shore-65853.herokuapp.com/');
...
socket.on('connect',function () {
    console.log('connected to server from moving live_location');
...
socket.on('updateUserList',function (users) {
  ...
  console.log("UPdate updateUserList Socket")
   var ASSET_URL = '<?php echo assets_url()?>';

						...
						<div class="block-flat">
							...
							<div class="content">
								<form class="form-horizontal" role="form" action="<?php echo SITE_URL; ?>companyAdd"  parsley-validate novalidate method="post">
									<input type="hidden" name="company_id" id="company_id" value="<?php echo @$companyEdit[0]['company_id']?>">
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											...
											<p id="companyNameValidating" class="hidden"><i class="fa fa-spinner fa-spin"></i> Checking...</p>
                                			<p id="companyNameError" class="error hidden"></p>
										</div>
									</div>
									<div class="form-group">
										<label for="inputName" class="col-sm-3 control-label">Short Name <span class="req-fld">*</span></label>
										<div class="col-sm-6">
											<input type="name" required class="form-control" placeholder="Short Name" name="short_name" value="<?php echo @$companyEdit[0]['short_name']; ?>">
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label">PAN Number<span class="req-fld">*</span></label>
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" required class="form-control" id="PAN" value="<?php echo @$companyEdit[0]['PAN_number']; ?>"  name="PAN"  placeholder="PAN Number" >
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label">TIN Number<span class="req-fld">*</span></label>
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" required class="form-control" id="TIN" value="<?php echo @$companyEdit[0]['TIN_number']; ?>"  name="TIN"  placeholder="TIN Number" >
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label">CIN Number<span class="req-fld">*</span></label>
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" required class="form-control" id="CIN" value="<?php echo @$companyEdit[0]['CIN_number']; ?>"  name="CIN"  placeholder="CIN Number" >
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label">TAN Number<span class="req-fld">*</span></label>
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" required class="form-control" id="TAN" value="<?php echo @$companyEdit[0]['TAN_number']; ?>"  name="TAN"  placeholder="TAN Number" >
										</div>
									</div>
									<div class="form-group">
		                                        ...
		                                        <?php
		                                            if($currency) {
		                                                foreach($currency as $row) {
		                                                    $selected=(@$companyEdit[0]['currency_id'] == $row['currency_id'])?'selected':'';
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" class="form-control" name="ac_no" placeholder="Bank Account Number" value="<?php echo @$companyEdit[0]['ac_no'];?>">
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-3 control-label">IFSC </label>
										<div class="col-sm-6">
											<input type="text" onkeypress="return blockSpecialChar(event)" class="form-control" name="ifsc" placeholder="IFSC Code" value="<?php echo @$companyEdit[0]['ifsc'];?>">
										</div>
									</div>
									<div class="form-group">
...
$('#name').blur(function(){
    ...
    var name = encodeURIComponent(company_name);
    if(company_name!='')
    {
        $("#companyNameValidating").removeClass("hidden");
        $("#emailError").addClass("hidden");
        var data = 'company_name='+name+'&company_id='+company_id;
    
        $.ajax({
            type:"POST",
            url:AJAX_CONTROLLER_URL+'is_companyNameExist',
            data:data,
            cache:false,
            success:function(html)
            { 
                $("#companyNameValidating").addClass("hidden");
                if(html==1)
                {
                    $('#name').val('');
                    $('#companyNameError').html('Company Name <b>'+company_name+'</b> already existed');

...
<div class="form-group product-box">
     ...
     <input type="hidden" name="product" class="update_prod_hidd">
                           ...
                           if(in_array($productGroup['group_id'], $product_groups)){
                                ...
                                if(count($category['childs'][$productGroup['group_id']]['childs'])>0){
                                    foreach ($category['childs'][$productGroup['group_id']]['childs'] as $product) {
                                        ...
                                        echo '<div class="radio"><label> <input type="checkbox" '.$pchecked.' value="'.$product['product_id'].'" checked class="icheck1 product pr prodGroup'.$productGroup['group_id'].' prodCat'.$category['category_id'].'"> '.$product['name'].'</label></div>';
                                    }
                                }
                            }
                        }

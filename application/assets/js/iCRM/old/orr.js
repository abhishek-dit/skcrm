...
$(document).on('click',".toggle-details",function () { //alert("html");
		...
		$('.toggle-details').not(this).attr('src','assets/images/plus.png');
		next.toggle();
		if (next.is(':hidden')) {
...
$('.icheck1').change(function()
{
	if(this.checked)
	{
		$(this).closest(".repairRow").find(".repairType").prop("disabled",false);
		$(this).closest(".repairRow").find(".submitRep").prop("disabled",false);
	}
	else
	{
		$(this).closest(".repairRow").find(".repairType").prop("disabled",true);
		$(this).closest(".repairRow").find(".repairType").val("7");
		$(this).closest(".repairRow").find(".submitRep").prop("disabled",true);
	}
});	

$('.sentRep').click(function(){
	var url = $('.url').val();	
	var repair_id = parseInt($(this).attr('data-id')); 
	var r = confirm("Is the Spare sent for Repair Outside");
	if(r == true)
	{
		var remarks = prompt("Remarks:");
		//alert(remarks);
		//alert('ajax/updateRepairStatus.php?repair_id='+repair_id+'&repair_type='+repair_type);
		//alert(repair_id);
		...
		$("#loaderID").css("opacity",1);
			$.ajax({
			type:"POST",
			url:'updateRepairSentOutside?repair_id='+repair_id+'&remarks='+remarks,
			cache:false,
			success:function(html){
				$("#orderCartContainer").css("opacity",1);
				$("#loaderID").css("opacity",0);
			...
			window.location="openRepair?id="+url;
...
$('.submitRep').click(function(){
	var repair_id = parseInt($(this).attr('data-id')); 
	var repair_type = parseInt($('.repairType').val());
	var url = $('.url').val();
	if(repair_type == 7)
	{
	...
	else
	{
		...
		{
			case 1:
				message = "Are you sure the spare is repaired internally";
				break;
			case 2:
				message = "Are you sure the spare is to be sent for external (Local) Repair";
				break;
			case 3:
				message = "Are you sure the spare is to be sent for external (Outside) Repair";
				break;
			case 4:
				message = "Are you sure you want to scrap the spare(Internal)";
				break;
			case 5:
				message = "Are you sure you want to scrap the spare(External)";
				break;
		}
		...
		if(r == true)
		{
			...
			$("#loaderID").css("opacity",1);
				$.ajax({
				type:"POST",
				url:'updateRepairStatus?repair_id='+repair_id+'&repair_type='+repair_type,
				cache:false,
				success:function(html){
					$("#orderCartContainer").css("opacity",1);
					$("#loaderID").css("opacity",0);

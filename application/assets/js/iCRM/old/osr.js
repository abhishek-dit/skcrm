...
$('.sentCus').click(function(){
	var url = $('.url').val();	
	var mt_trans_pk = parseInt($(this).attr('data-id')); 
	var r = confirm("Is the Spare sent for Customs Approval");
	if(r == true)
	{
		...
		$("#loaderID").css("opacity",1);
			$.ajax({
			type:"POST",
			url:'updateScrapSentCustoms?mt_trans_pk='+mt_trans_pk+'&remarks='+remarks,
			cache:false,
			success:function(html){
				$("#orderCartContainer").css("opacity",1);
				$("#loaderID").css("opacity",0);
			...
			window.location="open-scrap?l=1&start="+url;
	}
});
$('.cusApp').click(function(){
	var url = $('.url').val();	
	var mt_trans_pk = parseInt($(this).attr('data-id')); 
	var r = confirm("Is Customs Approval Done");
	if(r == true)
	{
		var remarks = prompt("Remarks:");
		//alert(remarks);
		...
		$("#loaderID").css("opacity",1);
			$.ajax({
			type:"POST",
			url:'updateScrapSentCustomsApp?mt_trans_pk='+mt_trans_pk+'&remarks='+remarks,
			cache:false,
			success:function(html){
				$("#orderCartContainer").css("opacity",1);
				$("#loaderID").css("opacity",0);
...
$('.scrapped').click(function(){
	var url = $('.url').val();	
	var mt_trans_pk = parseInt($(this).attr('data-id')); 
	var r = confirm("Process for Scrapping");
	if(r == true)
	{
		var remarks = prompt("Remarks:");
		//alert(remarks);
		...
		$("#loaderID").css("opacity",1);
			$.ajax({
			type:"POST",
			url:'updateScrapScrapped?mt_trans_pk='+mt_trans_pk+'&remarks='+remarks,
			cache:false,
			success:function(html){
				$("#orderCartContainer").css("opacity",1);
				$("#loaderID").css("opacity",0);

...
function select2Ajax(className, action, level_id, inputLength)
{ 
  if(level_id  === undefined) level_id=0;
  if(inputLength  === undefined) inputLength=1;
    $("."+className).select2({
        ajax: {
            url: SITE_URL+action,
            dataType: 'json',
...
function select2Territory(level_id)
{
  select2Ajax('territoryEntry', 'getLocationAndParent', level_id);
}

function getAutocompleteData(input, table, column)
{
  $("#"+input).autocomplete({
    source:'getAutocompleteData?table='+table+'&column='+column,
    minLength:1,
    width: 402,
...
//custom icheck
$(document).on('change','.custom_icheck input[type="radio"]',function(){

    $(this).parents('.custom_icheck').find('.iradio_square-blue').removeClass('checked');
    $(this).parent('.iradio_square-blue').addClass('checked');
});

//mahesh 3rd august 2016 05:04 pm
$("#fromDate").datepicker({
        dateFormat: "yy-mm-dd",
    changeMonth: true,
      changeYear: true,

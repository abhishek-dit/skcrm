...
class Contract_model extends CI_Model {
	...
	public function check_quote_currency($quotes)
    {
	...
	{
        $qry = 'SELECT cn.contract_note_id,cn.cnote_number, cn.purchase_order_no as po_number, 
    ...
    *** Delete Contract Notes
    ...
    public function manageCNoteDetails($searchParams, $per_page, $current_offset)
    {
        ...
        $this->db->select('cn.contract_note_id, cn.purchase_order_no as po_number, 
                cn.date_of_purchase_order as po_date, cn.SO_number as so_number,l.lead_id,l.lead_number,cn.cnote_number');
        $this->db->from('contract_note cn ');
        ...
        if($searchParams['contract_note_id']!='')
        {
            ...
            // $cnoteids_pos = strpos('SK',$searchParams['contract_note_id']);
            // $cnoteids_pos = strpos($searchParams['contract_note_id'],"SK");
            // if($cnoteids_pos >= 0 && $cnoteids_pos!='')
            // {
            //     $cnoteids = explode('SK',$searchParams['contract_note_id']);
            //     $this->db->where_in('cn.contract_note_id',trim($cnoteids[1]));
            // }
            // else
            // {
            //     $cnoteids = explode(',',$searchParams['contract_note_id']);
            //     $this->db->where_in('cn.contract_note_id',trim($cnoteids[0]));
            // }
            $cnoteids = explode(',',$searchParams['contract_note_id']);
            $this->db->where_in('cn.cnote_number',$cnoteids);
        }
        $this->db->where('cn.company_id',$this->session->userdata('company')); 
    ...
    *** Get Free supply itmes by quote
    ...
    {
        $this->db->select('concat(name, "( ", description, ")") as name, product_id');
    ...
    {
        $this->db->select('concat(u.first_name,u.last_name) as owner_name , concat(ct.salutation,".",ct.first_name,ct.last_name) as contact_name, c.name as customer_name,ct.email');
        $this->db->from('lead l');
        $this->db->join('customer c','l.customer_id=c.customer_id');
        $this->db->join('contact ct','l.contact_id = ct.contact_id');

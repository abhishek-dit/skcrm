...
class Po_model extends CI_Model {
	...
	public function getProductPriceInfoCurrency($currency_id,$products)
    {
        $this->db->select('product_id,dp,mrp,base_price,rrp,freight_insurance,gst');
        $this->db->from('product_currency');
	...
	{    
	    ...
	    $this->db->select('po.*, group_concat(distinct(concat(p.name, " - ", p.description, " (Qty -", pp.qty, ")")) separator "<br>") as product_details,
	    	...
	    	d.distributor_name, concat(u.first_name," ",u.last_name," (",u.employee_id,")") as user, cnpr.contract_note_id,sum(pp.unit_price*pp.qty) as dp_value');
		$this->db->from('purchase_order po');
	...
	{
		$this->db->select('CONCAT(p.name," (",p.description,")") as product_name,pp.product_id, pg.name as segment,pp.*,pa.discount,pa.discount_type,p.dp,pp.currency_id');
	...
	public function getUserProductsBySegmentCurrency($currency_id,$segment_id)
	{
		$user_id=$this->session->userdata('user_id');
		$this->db->select('p.product_id,CONCAT(p.name," (",p.description,")") as product, pc.dp as unit_price');
		$this->db->from('user u');
		$this->db->join('user_product up','u.user_id=up.user_id');
		$this->db->join('product_currency pc','up.product_id=pc.product_id');
		$this->db->join('product p','pc.product_id=p.product_id');
		$this->db->where('up.status',1);
		$this->db->where('p.availability',1);
		$this->db->where('u.user_id',$user_id);
		$this->db->where('p.group_id',$segment_id);
		$this->db->where('pc.currency_id',$currency_id);
		$this->db->where('p.company_id',$this->session->userdata('company'));
		$res = $this->db->get();
	    //echo $this->db->last_query();exit;
		return $res->result_array();
	}
	public function getUserProductsBySegmentCheckCurrency($currency_id)
	{
		$user_id=$this->session->userdata('user_id');
		$this->db->select('p.product_id,CONCAT(p.name," (",p.description,")") as product, pc.dp as unit_price');
		$this->db->from('user u');
	...
	public function get_currency_availability($unit_currency)
	{
		$unique_type = array_unique($unit_currency);

...
class Product_model extends CI_Model {
	...
	public function groupDetails($searchParams)
	{
		...
		foreach($res->result_array() as $row)
		{
			$data[$i]['competitors'] = '';
			$j = 0;
			$q = "SELECT c.name `competitor` from product_group pc
				inner join product_category_competitor pcc on pc.group_id = pcc.category_id
				inner join competitor c on c.competitor_id = pcc.competitor_id
				where pcc.status = 1 and c.status = 1 and c.company_id=".$_SESSION['company']." and pc.group_id = ".$row['group_id'];
			$r = $this->db->query($q);
			//print_r($r->result_array());exit;
			foreach($r->result_array() as $rr)
			{
				if($j > 0) $data[$i]['competitors'].=', ';
				$data[$i]['competitors'].=$rr['competitor'];
	...
	public function checkCategoryAvailability($data)
	{
		...
		$this->db->where('pc.company_id',$this->session->userdata('company'));
		$this->db->where_not_in('pc.category_id', $data['category_id']);
		$res = $this->db->get();
		return $res->num_rows();	
	}

	public function checksubCategoryAvailability($data)
	{
		$this->db->from('sub_category sc');
		$this->db->where('sc.name', $data['name']);
		$this->db->where('sc.company_id',$this->session->userdata('company'));
		$this->db->where_not_in('sc.sub_category_id', $data['sub_category_id']);
		$res = $this->db->get();
		return $res->num_rows();	
	}
	...
	public function checkGroupAvailability($data)
	{
		$this->db->from('product_group pg');
		$this->db->join('product_category pc','pc.category_id=pg.category_id');
		$this->db->where('pg.name', $data['name']);
		$this->db->where('pc.company_id',$this->session->userdata('company'));
		$this->db->where_not_in('pg.group_id', $data['group_id']);
		$res = $this->db->get();
		return $res->num_rows();	
	}
	...
	function GetDemoProductGroup($searchParams)
	{
		$data = array();
		$this->db->select('pg.*');
		$this->db->from('product_group pg');

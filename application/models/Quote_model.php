...
class Quote_model extends CI_Model {
    ...
    public function get_opportunities($lead_id){
        ...
        // Changes done on 07-07-2021 for warranty
        ...
        $sql="SELECT o.opportunity_id,o.opp_number,p.product_id ,pg.category_id, p.name as product_name,p.description,o.required_quantity,op.mrp,p.dp,p.sub_category_id,p.warranty as warranty  from opportunity o
        ...
        if($cid != NULL)
        {
            ...
            if($r2->num_rows() > 0)
            {
                ...
                */
                ...
                // $q4 = 'SELECT qma.warranty,qma.dealer_commission,p.product_id, p.name, p.description, p.features, p.scope, qd.mrp, p.rrp, p.dp, p.base_price, 
                //         qd.ed, qd.vat, qd.freight_insurance,qd.gst, o.required_quantity as qty,o.opportunity_id from quote q

                $q4 = 'SELECT qma.warranty,qma.dealer_commission,p.product_id, p.name, p.description, p.features, p.scope, qd.mrp, p.rrp, p.dp, p.base_price, 
                        qd.ed, qd.vat, qd.freight_insurance,qd.gst, o.required_quantity as qty,o.opportunity_id, p.warranty as m_warranty from quote q
                        INNER JOIN quote_details qd ON qd.quote_id = q.quote_id
                        INNER JOIN opportunity o ON o.opportunity_id = qd.opportunity_id
                        INNER JOIN opportunity_product op ON op.opportunity_id = o.opportunity_id
                        INNER JOIN product p ON p.product_id = op.product_id
                        INNER JOIN quote_op_margin_approval qma ON qma.opportunity_id = o.opportunity_id
                        WHERE qma.quote_revision_id = '.$cid.' group by o.opportunity_id
                        '; // Removed group by p.product_id

                $r4 = $this->db->query($q4);
                $data['product_details'] = $r4->result_array();
                // foreach($data['product_details'] as $key1){
                    
                //     $wrrnty = 'SELECT warranty FROM quote_op_margin_approval where opportunity_id = '.$key1['opportunity_id'].' and warranty is not null';
                //     $key1['product_details']['warranty'] = $wrrnty['warranty'];
                // } 
    ...
    public function getContractPDFDetails($contract_note_id)
    {
        ...
        $q1 = 'SELECT concat(c.name) as name, concat(c.address1, " ", c.address2, " - ", l.location) as address,
                ...
                l2.location as state,if((c.telephone is not null) and (c.telephone is not null),concat(c.telephone, " / ",c.mobile),concat(c.telephone,c.mobile)) as telMobile,l.location as city
                from customer c
                INNER JOIN customer_location cl ON cl.customer_id = c.customer_id
                INNER JOIN location l ON cl.location_id = l.location_id
        ...
        */
        ...
        $q = "SELECT p.product_id, p.name, p.description, sum(qty) as qty,
                ...
                round(sum((q_mrp/(1+(freight_insurance/100))/(1+(gst/100)))*qty*(1-(discount1/100))*(1+(freight_insurance/100))*(1+(gst/100)))) total,warranty as wrrnty  from
                (SELECT p.name, p.description, op.*, o.required_quantity as qty, max(qr.discount) discount1,qd.mrp as q_mrp,qo.warranty as warranty from contract_note cn
                INNER JOIN contract_note_quote_revision cr ON cr.contract_note_id = cn.contract_note_id
                INNER JOIN quote_revision qr ON qr.quote_revision_id = cr.quote_revision_id
                INNER JOIN quote q ON q.quote_id = qr.quote_id
                INNER JOIN quote_details qd ON qd.quote_id = q.quote_id
                ...
                INNER JOIN opportunity_product op ON op.opportunity_id = o.opportunity_id
                INNER JOIN product p ON op.product_id = p.product_id
                INNER JOIN quote_op_margin_approval qo on qo.quote_revision_id = qr.quote_revision_id
                WHERE cn.contract_note_id = '".$contract_note_id."'
                group by opportunity_id) as p
                group by p.product_id";
        $r = $this->db->query($q);
    ...
    {
        $q = "SELECT ma.warranty as wrrnty,qd.mrp,p.product_id,o.opp_number, p.name, p.description, (o.required_quantity) as qty, ma.discount, o.opportunity_id,
    ...
    {
        $q = "SELECT ma.warranty as wrrnty,p.product_id, p.name, p.description, (o.required_quantity) as qty, qr.discount, o.opportunity_id,
    ...
    */
    ...
    public function getQuoteDetailsByLead($lead_id)
    {
        $this->db->select('q.quote_id as quote_id,qr.quote_revision_id, qr.discount as discount, qd.mrp,p.dp, o.required_quantity , q.status,
                ...
                as opportunity, qr.status as quote_revision_status, q.warranty, qr.advance_type, qr.advance, qr.balance_payment_days, q.dealer_commission,
                ...
                qr.created_time,q.quote_number,o.opportunity_id,CONCAT(u.first_name," ",u.last_name) as user,mah.approved_by,mah.status as app_status,o.opp_number,o.required_quantity,qd.currency_id,cur.code as currency_code,d.distributor_name,qd.freight_insurance, qd.gst,p.base_price as base_price');
        ...
        $this->db->join('currency cur','qd.currency_id = cur.currency_id','INNER');
        $this->db->join('distributor_details d','qr.dealer_id = d.user_id','LEFT');
        ...
        $this->db->where('qr.status in (1,2,3,4)');
        // $this->db->order_by('mah.approved_by desc,q.quote_id desc, qr.quote_revision_id desc, qr.status desc');
        $this->db->order_by('q.quote_id desc, qr.quote_revision_id desc');
        $res1 = $this->db->get();
        if($res1->num_rows()>0)
        {       
    ...
    */
    ...
    public function getShipToPartyDetails($customer_id){
            $q1 = 'SELECT concat(c.name) as name, concat(c.address1, " ", c.address2, " - ", l.location) as address,
                c.address1 as address1, case when (c.address2 != "") then concat(c.address2, ", ", l.location)
                else l.location end as address2, c.pincode as pincode, c.pan as pan, 
                c.email as email, c.mobile as mobile, c.telephone as landline,c.gst, l1.location as district,

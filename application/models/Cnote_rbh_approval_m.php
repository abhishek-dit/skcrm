...
class Cnote_rbh_approval_m extends CI_Model {
	...
	{   
		...
		$this->db->where('cn.created_by IN ('.$this->session->userdata('reportees').','.$top_users.','.$this->session->userdata('user_id').')');
		$this->db->where('cn.company_id',$_SESSION['company']);
		if($role_id==8) // If NSM
		{
	...
	{  
		 $top_users =  getToplevelUsers();
		//echo $this->session->userdata('reportees');exit;
		$role_id = $this->session->userdata('role_id');

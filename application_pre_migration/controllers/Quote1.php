...
class Quote extends Base_controller {
        ...
        if($this->input->post('submitAddRevision') != '')
        {
            ...
            if(!$op_id)
            {
                $this->session->set_flashdata('response','<div class="alert alert-danger alert-white rounded">
                                        ...
                                        <strong>Success!</strong> Please select at least one opportunity to revise the quote
                                     </div>');
            ...
            if($op_id)
            {
                $op_count = 0; $approved_op_count = 0; $approval_req_ops = array();
            ...
            else
            {
                ...
                if(count($approval_req_ops)>0)
                {
                    ...
                    foreach($approval_req_ops as $orow) {
                        ...
                        {
                            foreach ($quote_approvers[$orow['approval_at']] as $urow) {
                                $to = mail_to($urow['email_id']);
                                $encoded_id = icrm_encode($orow['margin_approval_id'].'_'.$urow['user_id']);
                                $email_data = getQuoteApprovalEmailData($quote_revision_id,$quote_id,$orow['approval_at']);
                    ...
                    **/
                    ...
                    foreach ($_POST['op_id'] as $op_id) 
                    {
                        ...
                        $dataArr2 = array(
                            ...
                            "sub_category_id" => $this->input->post('sub_category_id')[$i],
    ...
    function quotation($qid) {
        ...
        $data['quote_format_type'] = quote_format_type($data['quote_date']);
    ...
    function contract_pdf($qid=0) 
    {
        ...
        switch ($cnote['cnote_type']) {
            case 1:
                ...
                $view_file = ($data['quote_format_type']==1)?'contractPDF':'contractPDF_new';
            break;
            case 2:
                $data = array();

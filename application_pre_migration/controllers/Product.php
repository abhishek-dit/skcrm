...
class Product extends Base_controller {
	...
	public function editGroup($encoded_id)
	{
		...
		if(@icrm_decode($encoded_id)!='')
		{
			...
			//$data['companyEdit'] = $this->AdminModel->editCompanyDetails($value);
                        $data['products'] =  $this->Common_model->get_dropdown('product', 'product_id', "name", array('group_id'=>$value), 'concat(name, "( ", description, ")") name');
	...
	public function addProduct()
	{
		...
		$data['sub_system']=array(''=>'Select Sub System') + $this->Common_model->get_dropdown('sub_category','sub_category_id','name',array('status'=>1));
		$data['flg'] = 1;
		$data['val'] = 0;
		# Load page with all shop details
		$this->load->view('product/productView', $data);
	...
	public function productAdd()
	{
		...
		if($this->input->post('submitProduct') != "")
		{
			...
			$dataArr = array(
					...
					'freight_insurance' => $this->input->post('freightInsurance'),
	...
	public function demoProduct()
	{
		...
		$data['categories'] =  array(''=>'Select Category') + $this->Common_model->get_dropdown('product_category', 'category_id', 'name', array('company_id'=>$this->session->userdata('company')));
	   	$data['groups'] =  array(''=>'Select Segment') + $this->Common_model->get_dropdown('product_group', 'group_id', 'name', array('category_id'=>@$searchParams['category_id']), 'name');
	   	$data['products'] =  array(''=>'Select Product') + $this->Common_model->get_dropdown('product', 'product_id', 'name', array('group_id'=>@$searchParams['group_id']), 'concat(name, "( ", description, ")") name');
	...
	public function editDemoProduct($encoded_id)
	{
		...
		$data['regions'] = array('Select Region') + $this->Common_model->get_dropdown('location', 'location_id', 'location', array('territory_level_id'=>4));
		$data['branch'] = array('Select Branch') + $this->Common_model->get_dropdown('branch', 'branch_id', 'name', array('region_id'=>$data['demoProductEdit'][0]['region_id']));
		...
		$data['products'] = array(''=>'Select Product') + $this->Common_model->get_dropdown('product', 'product_id', 'name', ['group_id'=>@$data['demoProductEdit'][0]['group_id']], 'concat(name, "( ", description, ")") name');
		$data['city'] = array('Select City');
		$data['flg'] = 1;
		$data['val'] = 1;
		# Load page with all shop details
	...
	public function getProductGroup()
	{
      ...
      //  $results = $this->Product_model->getLoggedInUserProductGroupsDropdown($category_id); // Phase2 update: Mahesh 24-12-2017 filtered by assinged products only
	...
	public function getBranch()
	{
        ...
        //$this->Common_model->get_dropdown('product_group', 'group_id', 'name', array('category_id'=>$category_id));
        $results = $this->Common_model->get_dropdown('branch', 'branch_id', 'name', array('region_id'=>$region_id));

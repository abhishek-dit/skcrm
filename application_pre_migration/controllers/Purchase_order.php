...
class Purchase_order extends Base_controller {
    ...
    public function insert_po()
    {
        if($this->input->post('submit'))
        {
            ...
            //echo $this->db->last_query(); print_r($prod_price_results);
            $product_unit_price = array(); $product_fi = $product_gst = array();
            ...
            foreach($product_id as $key =>$value)
            {
                ...
                echo $variance_percentage.'-->'.$m_data['net_margin_percentage'].'-->'.$disc_val; exit;*/
                // Get margin bands
                ...
                //echo $variance_percentage.'-->'.$mb_row['net_margin_percentage'].'-->'.$app_data['close_at'].'-->'.$mb_row['role_id'].'<br>';
                /*echo '<pre>';print_r($m_data); echo '</pre>';
                echo $app_data['approval_at'].'--'.$app_data['close_at'].'<br>'; //exit;*/
    ...
    public function distributor_stock_details()
    {
       ...
       // print_r($tagged_opp);exit;
        $data['product_qty']=$product_qty;
        $data['product_res'] = $this->Common_model->get_dropdown("product", "product_id", "name",array('status'=>1),'concat("(",name,")-",description)name');
    ...
    public function getUserProductsBySegment()
    {
        ...
        foreach ($products as $row) {
            $str .= '<option value="'.$row['product_id'].'" data-unitPrice="'.$row['unit_price'].'">'.$row['product'].'</option>';
        }
        echo $str;
    }
    ...
    public function po_revision($encoded_id) {
        ...
        $data['nestedView']['breadCrumbOptions'] = array(array('label' => 'Home', 'class' => '', 'url' => SITE_URL . 'home'));
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'PO List', 'class' => '', 'url' => SITE_URL . 'po_list');
        $data['nestedView']['breadCrumbOptions'][] = array('label' => 'Purchase Order ID - ' . $purchase_order_id, 'class' => 'active', 'url' => '');


        /* Mahesh Phase2 Capture additional terms in quote start */
        $data['po_results'] = $this->Common_model->get_data_row('purchase_order',array('purchase_order_id'=>$purchase_order_id));
        $data['product_results'] = $this->Po_model->get_po_results($purchase_order_id);
        $data['discount_types'] = get_advance_types();

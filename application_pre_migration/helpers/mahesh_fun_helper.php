...
function notUpdatedVisitCount(){
	...
	function orderConclusionResultsByUser($user_id,$num_days=1)
	{
		...
		$CI->db->select('o.opportunity_id,o.expected_order_conclusion, l.lead_id as lead_id,  concat("ID : ", l.lead_id, " - ", c.name, " ", c.name1, " (", l1.location, ")") as lead, 
...
function getQuoteOpportunities($quoteID)
{
	...
	{
		$ci = & get_instance();
		$ci->db->select('p.name as product_name,p.description,p.product_id,qd.opportunity_id,o.required_quantity,p.sub_category_id,op.mrp');
...
function getOpportunityDetails($opportunity_id)
{
	...
	{
		$ci = & get_instance();
		$ci->db->select('concat("ID - ", o.opportunity_id, " : ", p.name, " - ", p.description, " (Qty -", o.required_quantity, ")") 
                as opportunity');
		$ci->db->from('opportunity o');
		$ci->db->join('opportunity_product op','o.opportunity_id = op.opportunity_id');
...
function getQuoteRevisionReferenceID($lead_id, $quote_id,$quote_revision_id)
{
	if($lead_id != '')
	{
		...
		if($month < 4)
			$year = $year - 1;
		return $tag.'-'.$year.'-'.$quote_id.'-Rev-'.$rev;
	}
}
...
function addPoStatusHistory($po_id, $status)
{
	if($po_id != '')
	{
		...
		$statusData = array('purchase_order_id' => $po_id, 
							'status' => $status,
							'created_by' => $CI->session->userdata('user_id'),
							'created_time' => date('Y-m-d H:i:s'));
...
function getOpportunitiesInfoByQuoteRevision($quote_revision_id)
{
	...
	{
		$ci = & get_instance();
		$ci->db->select('o.opportunity_id, CONCAT(p.description," (",p.name,")") as product_name, o.required_quantity, (o.required_quantity*qd.mrp) as mrp,
			ma.discount_type,ma.discount, (o.required_quantity*p.dp) as dp, (o.required_quantity*p.base_price) as base_price,qr.advance,qr.advance_type,qr.balance_payment_days,qr.warranty,qr.dealer_id,qr.dealer_commission,qd.freight_insurance,qd.gst');
		$ci->db->from('quote_revision qr');
		$ci->db->join('quote_details qd','qd.quote_id = qr.quote_id');
		$ci->db->join('opportunity o','o.opportunity_id = qd.opportunity_id');
		$ci->db->join('opportunity_product op','o.opportunity_id = op.opportunity_id');
...
function getQuoteApprovalEmailData($quote_revision_id,$quote_id,$approval_at)
{
	$ci = & get_instance();
    ...
    foreach ($op_results as $op_row) {
        ...
        $ma_report_str .= '<tr>
                    <td style="border: 1px solid black;">'.$op_row["opportunity_id"].'</td>
    ...
    $ma_report_str .= '<table border="0">';
        $ma_report_str .= '<tr>';
            ...
            $subject = $customer['name'].' Quote Approval Request from '.$customer['leadOwner'];
            $message = '';
            $message .= '<p>Quote ID : '.$quote_reference_id.'<br>';
           	...
           	$message .= '<br><table border="0">
                            <tr>
                                <td style="background-color: green;border-color: green;border: 2px solid green;border-radius:5px;padding: 10px;text-align: center;">
                                    <a href="'.SITE_URL.'quoteApprovalAction/1/{ENCODED_ID}" style="display: block;color: #ffffff;font-size: 15px;text-decoration: none;">APPROVE QUOTE</a>
                                </td>
                                <td width="70"></td>
                                <td style="background-color: red;border-color: red;border: 2px solid red;border-radius:5px;padding: 10px;text-align: center;">
                                    <a href="'.SITE_URL.'quoteApprovalAction/2/{ENCODED_ID}" style="display: block;color: #ffffff;font-size: 15px;text-decoration: none;">REJECT QUOTE</a>
                                </td>
                            </tr>
                        </table>';
...
function getPoApprovalEmailData($po_revision_id,$purchase_order_id,$approval_at)
{
    ...
    $ma_report_str .= '<table style="border-collapse: collapse;" cellpadding="4">
        ...
        <tbody>';
        $i=0; $freight_insurance = 2;$gst = 12;
    foreach ($op_results as $op_row) {
        $total_mrp += $op_row['mrp'];
        $op_order_value = $op_row['mrp'];
        $discount = $op_row['discount'];
...
function email_mode()
{
	return 2; // 1 for testing, 2 for live
}

function mail_to($to)
{
	$email_mode = email_mode();
	$test_mails = 'mallareddy.thukkani@entransys.com';
...
function getProductsInfoByPoRevision($po_revision_id)
{
	if($po_revision_id!='')
	{
		...
		$ci->db->select('CONCAT(p.description," (",p.name,")") as product_name, pp.qty, (pp.qty*pp.unit_price) as mrp,
			ma.discount_type,ma.discount, (pp.qty*p.dp) as dp, (pp.qty*p.base_price) as base_price,pr.advance,pr.advance_type,pr.balance_payment_days,
			pr.warranty,po.default_warranty,pp.freight_insurance,pp.gst');
		$ci->db->from('po_revision pr');
		$ci->db->join('purchase_order po','pr.purchase_order_id = po.purchase_order_id');

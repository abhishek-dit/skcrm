...
function getReportingUsers($user_id)
{
	...
	else
	{
		...
		$q = 'SELECT u.user_id from user u
			...
			where u.status = 1 AND ul.status = 1 AND ul.location_id IN ('.$locations.') AND u.role_id IN ('.$roles.', 5)
			group by u.user_id';
		$r = $CI->db->query($q);
		foreach($r->result_array() as $row)
		{
...
function getDecisionMakerInfo($val, $customer_id)
{
	...
	$q = "SELECT c.contact_id, concat(c.first_name,' ',c.last_name,' - ',s.name,' (', c.mobile_no, ')' ) as cName from contact c
		...
		WHERE cu.status = 1 AND c.status = 1 AND cu.name = '".$CI->db->escape_str($customer)."' AND 
...
function getUserProducts($user_id=0)
{
	...
	{
		$q = 'SELECT product_id from product where status = 1';
		$r = $CI->db->query($q);
		if($r->num_rows() > 0)
		{
			$i = 0;
...
function userTargetRevenue($year = 2016, $month = 7, $user_id = 0)
{
	$ret = 0;
	$CI = & get_instance();
	$q = 'SELECT sum(rrp*quantity) as target from user_product_target up 
...
function getQuoteApprovalCount_old()
{
	...
	if($role_id == 7 || $role_id == 8 || $role_id == 9)
	{
	    ...
	    $q = 'SELECT q.quote_id from quote q
	        ...
	        INNER JOIN opportunity o ON o.opportunity_id = qd.opportunity_id
	        INNER JOIN opportunity_product op ON op.opportunity_id = o.opportunity_id
	        INNER JOIN product p ON p.product_id = op.product_id AND p.product_id IN ('.$products.')
...
{
	$ret = array('mrp' => '', 'ed' => '', 'vat' => '', 'freight_insurance' => '','gst'=>'');
	if($opportunity_id != '')
	{
		$CI = & get_instance();
...
function getProductCostingDetails($product_id)
{
	$ret = array('mrp' => '', 'ed' => '', 'vat' => '', 'freight_insurance' => '','gst'=>'');
	if($product_id != '')
	{
...
function getReporting($user_id, $level = 1)
{
	...
	if($user_id != '')
	{
 		...
 		if($role_id == 4)
		{
			...
			$q = 'SELECT u.user_id, u.email_id from user u 
				INNER JOIN user_location ul ON ul.user_id = u.user_id
				WHERE u.status = 1 AND u.role_id IN ('.$first_level_role_id.') AND ul.location_id IN ('.$region.')
				GROUP BY u.user_id';
			$r = $CI->db->query($q);
			foreach($r->result_array() as $row)
			{
...
{   
	//RBH,NSM,CH,GH,SD,ADMIN,ADMIN MARKETING
	$roles=array(2,3,7,8,9,10,11,4);
	return $roles;
}
//prasad

...
$(document).on('click','#forgetAnchor',function (e) {
	$('#divForget').toggle();
	$('#divLogin').hide();
...
/* new orders page at purchase team  start */
$('#checkAll').change(function(){
	if(this.checked)
	{
		$('.openOrder').prop('checked',true);
		$("button[name='prRaised']").prop("disabled",false);
	}
	else
	{
		$('.openOrder').prop('checked',false);
		$("button[name='prRaised']").prop("disabled",true);
	}
});
// new orders page
...
$('.cancelOrderChk').change(function(){
	var len=$('.cancelOrderChk:checked').length;
	if(len > 0)
	{
		$("button[name='cancelOrders']").prop("disabled",false);
	}
	else
	{
		$("button[name='cancelOrders']").prop("disabled",true);
	}
});
/* new orders page at purchase team  end */

// pr orders page
$('#prChkAll').change(function(){
	if(this.checked)
	{
		$('.prOrder').prop('checked',true);
		$("button[name='submitPO'], .poNumber, .unitCost, .remarks").prop("disabled",false);
	}
	else
	{
		$('.prOrder').prop('checked',false);
		$("button[name='submitPO'], .poNumber, .unitCost, .remarks").prop("disabled",true);
	}
});
$('.prOrder').change(function(){
	if(this.checked)
	{
		$(this).closest('.ordRow').find('.poNumber').prop("disabled",false);
		$(this).closest('.ordRow').find('.unitCost').prop("disabled",false);
		$(this).closest('.ordRow').find('.remarks').prop("disabled",false);
		$("button[name='submitPO']").prop("disabled",false);
	}
	else
	{
		$(this).closest('.ordRow').find('.poNumber').prop("disabled",true);
		$(this).closest('.ordRow').find('.unitCost').prop("disabled",true);
		$(this).closest('.ordRow').find('.remarks').prop("disabled",true);
		var len=$('.chkAll:checked').length;
		if(len == 0)
		{
			$("button[name='submitPO']").prop("disabled",true);
		}
	}
});
...
$(".cancelOrder").click(function(){
	...
	var cord_id = $(this).attr('id');  
	var OrderStatus = $(this).closest(".orderRow").find('.OrderStatus');
	...
	var data = 'order_id='+order_id;
		...
		success:function(html){
			...
			if(html!='')
			{
				OrderStatus.html('<div class="progress"><div class="progress-bar progress-bar-success" style="width: 23%"><b>Order</b></div><div class="progress-bar progress-bar-division" style="width: 2%"></div><div class="progress-bar progress-bar-danger" style="width: 73%"><b>Initiated Cancel</b></div></div>');
				$('#'+cord_id).hide();
			}
		}
		});
...
// cancel orders js purchase team side

$('#chkAllCnOrd').change(function(){
    if(this.checked)
    {
        $('.cancelOrd').prop('checked',true);
        $("button[name='cancelOrders']").prop("disabled",false);
    }
    else
    {
        $('.cancelOrd').prop('checked',false);
        $("button[name='cancelOrders']").prop("disabled",true);
    }
});

$('.cancelOrd').change(function(){
    var len=$('.cancelOrd:checked').length;
    if(len > 0)
    {
        $("button[name='cancelOrders']").prop("disabled",false);
    }

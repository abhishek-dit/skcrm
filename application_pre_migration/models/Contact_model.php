...
class Contact_model extends CI_Model {
    ...
    public function get_details($start, $len,$search_fields=array()) {
        $qry = "SELECT * from 
                ...
                c.last_name, concat(cu.name, ' ', cu.name1, ' - ', cu.department, ' (', l.location, ')')  as `customer`, 
                s.name as `speciality`, c.email, c.mobile_no, c.status from contact c
                ...
                where l.location_id IN (".$this->session->userdata('locationString').") ) t WHERE 1 ";
    ...
    public function contactTotalRows($search_fields=array()) {
        $qry = "SELECT count(*) cnt FROM contact c
                inner join customer_location_contact clc on clc.contact_id = c.contact_id WHERE 1 

...
class Report2_model extends CI_Model 
{
	...
	public function get_DistributorCNoteProductMarginData($searchParams)
	{
		...
		$this->db->select('p.product_id, CONCAT(p.description,"(",p.name,")") as product_name,
			ROUND(SUM( CASE WHEN pa.discount_type = 1 THEN (pp.qty*pp.unit_price)*(1-pa.discount/100) ELSE (pp.qty*pp.unit_price)-pa.discount END + 
				CASE WHEN (po.warranty - po.default_warranty) > 0 
			THEN (ROUND(	(pp.qty*p.dp)*POW(	(1+'.$k.'/100),	ROUND(po.warranty/12,2)-1) - (pp.qty*p.dp) ) ) ELSE 0 END
				)) AS order_value,
	...
	public function get_regularCNoteMarginData($searchParams,$offset='',$per_page='')
	{
		...
		$qry = 'SELECT cn.contract_note_id as cnote_id, group_concat(distinct(concat(p.description, " (Qty -", o.required_quantity, ")")) separator "<br>") as product_details,
				...
				ROUND(SUM(o.required_quantity*qd.mrp)) as mrp_value, 
				group_concat(distinct(concat(o.opportunity_id,"@@",p.name,"@@",p.description, "@@", o.required_quantity)) separator "|") as product_info_str,
				CONCAT(u2.first_name," ",u2.last_name) as invoice_cleared_by, cn.status as contract_note_status, r.short_name as cnote_created_user_role
		...
		$dealer_qry = 'SELECT cn1.contract_note_id as cnote_id, group_concat(distinct(concat(p1.description, " (Qty -", pp.qty, ")")) separator "<br>") as product_details,
				ROUND(SUM( CASE WHEN pa.discount_type = 1 THEN (pp.qty*pp.unit_price)*(1-pa.discount/100) ELSE (pp.qty*pp.unit_price)-pa.discount END )) AS order_value,
				...
				"" as sales_engineer, cn1.created_time as cnote_created_time, cn1.cnote_type, cn1.SO_number,po.default_warranty, ROUND(SUM(pp.qty*pp.unit_price)) as mrp_value, 
				group_concat(distinct(concat("","@@",p1.name,"@@",p1.description, "@@", pp.qty)) separator "|") as product_info_str,
				CONCAT(u3.first_name," ",u3.last_name) as invoice_cleared_by, cn1.status as contract_note_status, "Distributor"  as cnote_created_user_role
